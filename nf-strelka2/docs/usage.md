...
```bash
nextflow pull afkoeppel/NF-Strelka2
```

### Reproducibility
It's a good idea to specify a pipeline version when running the pipeline on your data. This ensures that a specific version of the pipeline code and software are used when you run your pipeline. If you keep using the same tag, you'll be running the same version of the pipeline, even if there have been changes to the code since.

First, go to the [NF-Strelka2 releases page](https://github.com/afkoeppel/NF-Strelka2/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
...
* `docker`
    ...
    * Runs using the `local` executor and pulls software from dockerhub: [`nf-strelka2`](http://hub.docker.com/r/nf-strelka2/)
* `aws`
